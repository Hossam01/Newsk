...
public class EntertainmentFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        final View rootView = inflater.inflate(R.layout.entertainment_fragment, container, false);

        mSwipeRefreshLayout = (SwipeRefreshLayout) rootView.findViewById(R.id.swipe_refresh_layout4);
        gridView = (GridView) rootView.findViewById(R.id.gridView4);
        arrayitems = new ArrayList<Listitem>();

        FirebaseDatabase database = FirebaseDatabase.getInstance();
        final DatabaseReference myRef = database.getInstance().getReference().child("Entertainment");
        myRef.keepSynced(true);
        myRef.removeValue();

        url = "https://api.myjson.com/bins/16ptap";
                ...
                try {
                    ...
                    for (int i = 0; i < jarray.length(); i++) {
                        ...
                        try {
                            ...
                            e.printStackTrace();
                        } // hat el 7etta ely feha el firebase query (push)
                        arrayitems.add(item);
